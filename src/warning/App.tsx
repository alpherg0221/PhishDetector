    ...
    <>
      <ThemeProvider style={ {
        ...
        <Stack tokens={ { childrenGap: 20 } } style={ { paddingTop: "200px" } }>
          ...
          <Text style={ { color: "#e9e9e9", fontSize: "2.5em" } }>フィッシングサイトを検出しました</Text>
          <Text style={ { color: "#e9e9e9", fontSize: "1.25em" } }>
            <b>{ (new URL(location.href)).searchParams.get("url") }</b> は機密情報を盗み取る疑いがあります
          </Text>
          <Text style={ { color: "#e9e9e9", fontSize: "1.25em" } }>
          ...
          <Stack horizontalAlign="center" style={ { paddingTop: 20 } }>
            <DefaultButton onClick={ closePage }>ページを閉じる</DefaultButton>
          </Stack>
        </Stack>
      </ThemeProvider>
    </>
